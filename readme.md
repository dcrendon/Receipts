...
# Start the tool
deno task start
# Start the tool and ignore certificate errors
deno task unsafe
```

## Configuration

All configuration is via `.env`. When run interactively in a terminal, a setup
wizard will prompt for missing credentials and confirm before running.
...
A provider runs only when all its required fields are present. Missing providers
